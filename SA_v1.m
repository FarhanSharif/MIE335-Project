function [z, w, b, s] = SA_v1(M, H, mu, x_0, T_0, alpha, N, P)
...
for k = 0:N
    for jj = 0:P
        ...
        for m = 1:x_dim
            % try moving one dimension at a time for higher chance of a
            % better point
            % u_cand in [-1/sqrt(N), 1/sqrt(N)], so the avg norm of a
            % vector of these is 1
            u_cand = (rand*(2/sqrt(x_dim)))-(1/sqrt(x_dim));
            x_cand = x_curr;
            x_cand(m) = x_cand(m)+T_curr*u_cand;
