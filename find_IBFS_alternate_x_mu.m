function [tableau, vars_in_basis, complementary_positions, basis_lookup] = find_IBFS_alternate_x_mu(A, b, combo_code, x_dim)
...
% If RHS = 0, then add the corresponding "x" or "mu" variable depending on
% the row and combo_code (4 different combinations are generated: basis
% with only x's, basis with only mu's, alternating x1, mu2, x3, mu4..., and
% alternating mu1, x2, mu3, x4... These 4 combinations are chosen because
% they are easiest to generate.
for ii = 1 : num_x
    ...
    if b(ii) ~= 0
        ...
        % add rows corresponding "x" var to basis
        if combo_code == 1
            vars_in_basis(ii) = ii; 
        
        % add rows corresponding to "mu" var to basis
        elseif combo_code == 2
            vars_in_basis(ii) = x_dim(1) + x_dim(2) + ii;
        
        % add rows corresponding to "x" and "mu" (x1, mu2, x3, mu4...)
        elseif combo_code == 3
            if mod(ii, 2) == 1
                vars_in_basis(ii) = ii;
        ...
        % add rows corresponding to "mu" and "x" (mu1, x2, mu3, x4...)
        elseif combo_code == 4
            if mod(ii, 2) == 1
                vars_in_basis(ii) = x_dim(1) + x_dim(2) + ii;
